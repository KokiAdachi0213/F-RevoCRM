...
$htmlEventAttributes = "onerror|onblur|onchange|oncontextmenu|onfocus|oninput|oninvalid|onresize|onauxclick|oncancel|oncanplay|oncanplaythrough|".
    ...
    if ($replaceAll) {
        $regex = '\s*[=&%#]\s*(?:"[^"]*"[\'"]*|\'[^\']*\'[\'"]*|[^]*[\s\/>])*/i';
        $result = preg_replace("/\s*(" . $htmlEventAttributes . ")" . $regex, '', $value);
        if ($result === null) {
            error_log('purifyHtmlEventAttributes: preg_replace failed (error=' . preg_last_error() . ')');
        } else {
            $value = $result;
        }
...
function purifyScript($value){
    // HTMLPurifier通過後に生のscriptタグが残っている時点で異常事態のため、
    // scriptタグ以降のすべてのコンテンツを信頼できないものとして全削除する（防御の深層化）
    // エンティティ形式（&lt;script&gt;）はHTMLPurifierが安全と判断した表示用テキストのため対象外
    $scriptRegex = '/<script\b[\w\W]*/i';
    ...
    foreach($restrictJavascriptInTags as $tag){
        ...
        if (preg_match_all('/(&.*?lt;|<)'.$tag.'[^>]*?(>|&.*?gt;)/i', $value,$matches)) {
            ...
            foreach($matches[0] as $matchedValue){
                ...
                if ($purifyContent === null) {
                    error_log('purifyJavascriptAlert: preg_replace for entity replacement failed (error=' . preg_last_error() . ')');
                    $purifyContent = $matchedValue;
                }
                $result = preg_replace($javaScriptRegex,"<$tag>",$purifyContent);
                if ($result === null) {
                    error_log('purifyJavascriptAlert: preg_replace for javascript check failed (error=' . preg_last_error() . ')');
                    $purifyContent = $matchedValue;
                } else {
                    $purifyContent = $result;
                }
